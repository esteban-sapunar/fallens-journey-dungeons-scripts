...
public class TownCardPanelController : MonoBehaviour
{
    ...
    public GameObject buttonRemoveType;
    public GameObject buttonMoveType;
    ...
    public void ClosePanel(){
        ...
        buttonRemoveType.SetActive(false);
        buttonMoveType.SetActive(false);
        gameObject.SetActive(false);
    }

    public void OpenPanel(Card newCard,GameObject newslot,bool fromDesk,bool fromTrainer){
        card = newCard;
        slot = newslot;
        comeFromDesk = fromDesk;
        name.text = card.name;
        cost = card.price;
        priceNumber.text = cost.ToString();
        ...
        if(fromDesk){
            ...
            buttonRemoveType.SetActive(true);
    ...
    public void RemoveType(){
        TownDesk.instance.RemoveType(card);
        ClosePanel();
    }

    public void AddType(){
        TownDesk.instance.AddType(card);
        ClosePanel();
    }

    public void SellCard(){
        if(TownDesk.instance.desk.Count+CardsVault.instance.cards.Count >30){
            player.GetComponent<TownPlayerStats>().EarnCardPoints(cost);
            if(comeFromDesk){
                TownDesk.instance.Remove(card);
            }
            else{
    ...
    public void ConfirmSellCard(){
        validationPanel.SetActive(true);
        validationPanel.GetComponent<ValidationPanelController>().delegatedMethodCall = SellCard;
    }
